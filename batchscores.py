...
def DeepNormalityHyperspheres_BatchScores_DMSVDD(test_loader, device, net, hyperspheres_center, radius):
    ...
    with torch.no_grad():
        for index, (data, targets) in enumerate(test_loader):
            ...
            dist_to_centers = torch.sum((outputs[0].unsqueeze(1).repeat(1, hyperspheres_center.size()[0],1)
                                         - hyperspheres_center.unsqueeze(0).repeat(outputs[0].size()[0], 1, 1)) ** 2, dim=2) # [512,100]
            try:
                scores_per_center = torch.cat([scores_per_center, dist_to_centers], dim=0)
...
def DeepNormalityHyperspheres_BatchScores_DMSVDD_cifar(test_loader, device, net, hyperspheres_center, radius):
    scores = []
    scores_labels = []
    net.eval()
    with torch.no_grad():
        for index, (data, targets) in enumerate(test_loader):
            ...
            try:
                ...
                scores_per_center = dist_to_centers # 512
