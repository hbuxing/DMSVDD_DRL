...
class Encoder(nn.Module):
    ...
    def _s_re(self, mu, log_sigma, v):
        ...
        std_z = torch.from_numpy(np.random.normal(0, 1, size=sigma.size())).float().to(DEVICE) # εc
        e = torch.tensor(np.random.normal(0, 1, sigma.size()), dtype=torch.float).to(DEVICE) # εv
        z1 = (v / 2 - 1 / 3) * torch.pow(1 + (e / torch.sqrt(9 * v / 2 - 3)), 3)
        # reparameterization trick
        z2 = torch.sqrt(v / (2 * z1))
        z = mu + sigma * z2 * std_z # zc
...
class GMM(nn.Module):
    ...
    def get_asign(self, z):
        ...
        yita_c = yita / (yita.sum(1).view(-1, 1))
